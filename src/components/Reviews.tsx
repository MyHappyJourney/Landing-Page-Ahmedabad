...
import { Star, ChevronLeft, ChevronRight, CheckCircle, MapPin, Sparkles } from 'lucide-react';
    ...
    <section id="reviews-section" className="py-12 sm:py-16 bg-[#F6F7F6]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-3xl mx-auto mb-8 sm:mb-12">
          <div className="inline-flex items-center gap-1.5 bg-white px-3.5 py-1.5 rounded-full border border-gray-200 shadow-xs mb-2">
            ...
            <span className="text-xs font-bold text-gray-800">4.9 ★ Rated on Google</span>
            <span className="text-gray-300">•</span>
            <span className="text-xs font-semibold text-emerald-700">200+ Gujarat Travellers</span>
          </div>
          ...
          <p className="text-xs sm:text-base text-gray-600 mt-2">
            Real verified reviews from families & couples across Ahmedabad and Gujarat.
          </p>
        </div>
        ...
        <div className="hidden lg:grid lg:grid-cols-3 gap-6">
          {REVIEWS.map((rev) => (
            <div
              key={rev.id}
              className="bg-white rounded-2xl p-6 shadow-xs border border-gray-200 flex flex-col justify-between hover:shadow-md hover:border-blue-200 transition-all"
            >
              <div>
                {/* Google Logo & Rating + Trip Badge */}
                <div className="flex items-center justify-between mb-3">
                  ...
                  {rev.tripType ? (
                    <span className="text-[10px] font-bold text-[#0B3996] bg-[#EBF2FF] px-2.5 py-1 rounded-full flex items-center gap-1">
                      <Sparkles className="w-3 h-3 text-[#0B3996]" />
                      {rev.tripType}
                    </span>
                  ) : (
              ...
              <div className="pt-4 border-t border-gray-100 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-full bg-[#0B3996] text-white font-black text-sm flex items-center justify-center shrink-0 shadow-xs">
                    {rev.name.charAt(0)}
                  </div>
                  <div>
              ...
              >
                <div>
                  <div className="flex items-center justify-between mb-3">
                    ...
                    {rev.tripType ? (
                      <span className="text-[9px] font-bold text-[#0B3996] bg-[#EBF2FF] px-2 py-0.5 rounded-full">
                        {rev.tripType}
                      </span>
                    ) : (
                      <span className="text-[10px] font-bold text-emerald-700 flex items-center gap-1">
                        <CheckCircle className="w-3 h-3 text-emerald-600 inline" /> Verified
                      </span>
                    )}
                  </div>
                ...
                <div className="pt-3 border-t border-gray-100 flex items-center justify-between">
                  <div className="flex items-center gap-2.5">
                    <div className="w-8 h-8 rounded-full bg-[#0B3996] text-white font-bold text-xs flex items-center justify-center shrink-0">
                      {rev.name.charAt(0)}
                    </div>
                    <div>
